...
function checkChronological(date1, date2, time1, time2) {
    ...
    function createDateObject(date, time) {
        const dateObject = new Date(`${date} ${time || '00:00'}`);
        return dateObject;
    }
}
...
function checkOriginDestinationsAreChronological(originDestinations) {
    ...
    for (let i = 0; i < originDestinations.length - 1; i++) {
        ...
        const curDateTimeRange = (cur.departureDateTimeRange != null) ? cur.departureDateTimeRange : cur.arrivalDateTimeRange;
        const nextDateTimeRange = (next.departureDateTimeRange != null) ? next.departureDateTimeRange : next.arrivalDateTimeRange;
...
//# sourceMappingURL=FlightSearchBody.js.map